...
class ValidationResult:
    ...
    def __init__(self, is_valid: bool, warnings: list[str] | None = None, errors: list[str] | None = None):
...
def validate_slides(slides: list[Path], output_dir: Path, min_disk_space_gb: float = 10.0) -> ValidationResult:
    """
    ...
    if missing_files:
        errors.append(f"{len(missing_files)} slide file(s) not found: {', '.join(f.name for f in missing_files[:3])}")
    
    # Check file formats (basic check - just extension)
    supported_extensions = {'.tif', '.tiff', '.ome.tif', '.ome.tiff', '.svs', '.ndpi', '.vsi', '.czi', '.scn'}
    unsupported_files = [s for s in slides if s.suffix.lower() not in supported_extensions]
    ...
    if existing_slides:
        ...
        try:
            disk_usage = shutil.disk_usage(output_dir.parent if output_dir.exists() else Path.cwd())
            ...
            if available_gb < min_disk_space_gb:
                errors.append(f"Insufficient disk space: {available_gb:.1f} GB available, {min_disk_space_gb:.1f} GB minimum required")
        ...
        large_slides = [s for s in existing_slides if s.stat().st_size > 5 * 1024**3]  # > 5GB
        if large_slides:
            warnings.append(
