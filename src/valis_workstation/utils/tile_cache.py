...
class LRUTileCache:
    ...
    def get(self, key: TileKey) -> Optional[np.ndarray]:
        ...
        with self._lock:
            if key in self._cache:
                ...
                self._hits += 1
                logger.debug(
                    f"Cache hit: {key.slide_path} L{key.level} ({key.tile_x}, {key.tile_y})"
                )
                return self._cache[key]
            else:
                self._misses += 1
                logger.debug(
                    f"Cache miss: {key.slide_path} L{key.level} ({key.tile_x}, {key.tile_y})"
                )
                return None
    ...
    def put(self, key: TileKey, tile_data: np.ndarray) -> None:
        ...
        with self._lock:
            ...
            while (
                self._memory_usage + tile_bytes > self.max_memory_bytes and self._cache
            ):
    ...
    def clear_slide(self, slide_path: str) -> None:
        ...
        with self._lock:
            keys_to_remove = [
                key for key in self._cache.keys() if key.slide_path == slide_path
            ]
...
class TiledImageLoader:
        ...
        if self._reader is None:
            with self._reader_lock:
                if self._reader is None:  # Double-check after acquiring lock
                    ...
                    logger.info(
                        f"Initialized slide reader: {self._reader.__class__.__name__}"
                    )
    ...
    def get_tile(
        ...
        try:
            ...
            if hasattr(reader, "metadata") and hasattr(
                reader.metadata, "level_dimensions"
            ):
                level_dims = reader.metadata.level_dimensions[level]
            else:
                # Fallback: use level 0 dimensions with downsample
                base_dims = (
                    reader.metadata.slide_dimensions[0],
                    reader.metadata.slide_dimensions[1],
                )
                downsample = 2**level
            ...
            if pixel_x >= level_dims[0] or pixel_y >= level_dims[1]:
                ...
                logger.debug(
                    f"Tile ({tile_x}, {tile_y}) outside bounds at level {level}"
                )
                return None
            ...
            if hasattr(reader.slide, "read_region"):
                ...
                tile_pil = reader.slide.read_region(
                    (pixel_x, pixel_y), level, (actual_width, actual_height)
                )
            ...
            if actual_width < self.tile_size or actual_height < self.tile_size:
                padded = np.zeros(
                    (self.tile_size, self.tile_size, tile_data.shape[2]),
            ...
            logger.error(
                f"Failed to load tile ({tile_x}, {tile_y}) at level {level}: {e}"
            )
            return None
    ...
    def get_region(
        ...
        region = full_region[offset_y : offset_y + height, offset_x : offset_x + width]
