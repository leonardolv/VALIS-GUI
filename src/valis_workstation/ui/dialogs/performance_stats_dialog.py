...
class PerformanceStatsDialog(QtWidgets.QDialog):
    ...
    def _update_stats(self):
        ...
        try:
            ...
            self._thumb_misses_label.setText(str(monitor.metrics.thumbnail_cache_misses))
            self._thumb_generated_label.setText(str(len(monitor.metrics.thumbnail_load_times)))
            self._thumb_avg_time_label.setText(f"{thumb_load_time:.1f} ms")
            
            # Thumbnail cache progress bar
            if thumb_stats['max_size_mb'] > 0:
                thumb_pct = int((thumb_stats['size_mb'] / thumb_stats['max_size_mb']) * 100)
                self._thumb_cache_bar.setValue(thumb_pct)
                self._thumb_cache_bar.setFormat(f"{thumb_stats['size_mb']:.1f} / {thumb_stats['max_size_mb']:.1f} MB ({thumb_pct}%)")
            ...
            if tile_stats['memory_limit_mb'] > 0:
                tile_pct = int((tile_stats['memory_usage_mb'] / tile_stats['memory_limit_mb']) * 100)
                self._tile_cache_bar.setValue(tile_pct)
                self._tile_cache_bar.setFormat(f"{tile_stats['memory_usage_mb']:.1f} / {tile_stats['memory_limit_mb']:.1f} MB ({tile_pct}%)")
        ...
        if reply == QtWidgets.QMessageBox.StandardButton.Yes:
            try:
                ...
                QtWidgets.QMessageBox.information(
                    self,
                    "Cache Cleared",
                    "Tile cache has been cleared successfully."
                )
