...
class MergeSlidesDialog(QtWidgets.QDialog):
    ...
    def __init__(self, slide_names: list[str], parent: QtWidgets.QWidget | None = None) -> None:
        ...
        self._table.setHorizontalHeaderLabels(["Include", "Slide Name", "Channel Name", "Color"])
        ...
        for i, name in enumerate(slide_names):
            ...
            color_combo.addItems([
                "Auto", "Red", "Green", "Blue", "Cyan", "Magenta", 
                "Yellow", "Gray", "White"
            ])
    ...
    def get_merge_config(self) -> dict:
        ...
        for i in range(self._table.rowCount()):
            ...
            if not isinstance(include_cb, QtWidgets.QCheckBox) or not include_cb.isChecked():
                continue
            
            slide_name = self._table.item(i, 1).text()
            channel_name = self._table.item(i, 2).text()
            color_combo = self._table.cellWidget(i, 3)
            color = color_combo.currentText() if isinstance(color_combo, QtWidgets.QComboBox) else "Auto"
