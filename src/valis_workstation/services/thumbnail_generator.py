...
def generate_thumbnail(
    ...
    try:
        ...
        if hasattr(slide_reader, 'slide') and hasattr(slide_reader.slide, 'associated_images'):
        ...
        if thumbnail_array is None:
            try:
                ...
                logger.info(f"Generating thumbnail from level {lowest_level}/{n_levels-1}")
                ...
                if not isinstance(thumbnail_array, np.ndarray):
                    ...
                    if hasattr(thumbnail_array, 'numpy'):
                        ...
                        thumbnail_array = np.ndarray(
                            ...
                            dtype=np.uint8,
                            shape=[thumbnail_array.height, thumbnail_array.width, thumbnail_array.bands]
                        )
...
def _numpy_to_pixmap(array: np.ndarray, max_size: int) -> QtGui.QPixmap:
    ...
    if array.ndim == 2:
        ...
        if channels == 1:
            ...
            logger.warning(f"Multi-channel image ({channels} channels), using first 3 as RGB")
            array = array[:, :, :3]
            bytes_per_line = width * 3
            qimage = QtGui.QImage(
                array.data,
